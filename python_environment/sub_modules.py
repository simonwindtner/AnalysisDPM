...
class ContextNetwork(nn.OutputSplitsLayer):
    ...
    # def mycall(self, *inputs, **kwargs):
    #     """Wrap the layer's __call__() with dictionary inputs and outputs.

    #     IMPORTANT: If no input_keys are  ovided to the constructor, they are
    ...
    #     outputs = super().super().super().__call__(*inputs, **kwargs)
    ...
    def compute_output(self, conditioning, pedal, z):
        ...
        """
        if "modify_conditioning_matrix_at_context_input" in self.modification_functions:
            conditioning = tf.constant(self.modification_functions["modify_conditioning_matrix_at_context_input"](conditioning.numpy()))
        # conditioning_var = tf.Variable(conditioning)
        # conditioning_var[:,:,0].assign(0.2 * conditioning_var[:,:,0])
        # conditioning_const = tf.constant(conditioning_var)
        # conditioning = conditioning_const
...
class MonophonicNetwork(nn.OutputSplitsLayer):
    ...
    def compute_output(self, conditioning, extended_pitch, context):
        ...
        if "modify_conditioning_matrix_at_monophonic_input" in self.modification_functions:
            conditioning = tf.constant(self.modification_functions["modify_conditioning_matrix_at_monophonic_input"](conditioning.numpy()))
...
class MultiInstrumentReverb(nn.DictLayer):
    ...
    def call(self, piano_model) -> ['reverb_ir']:
        ...
        if "modify_ir" in self.modification_functions:
            ir = self.modification_functions["modify_ir"](ir.numpy())
