...
class PianoModel(Model):
    ...
    @property
    def modification_functions(self):
        return self._modification_functions

    @modification_functions.setter
    def modification_functions(self, value):
        self._modification_functions = value
        self.context_network.modification_functions["modify_conditioning_matrix_at_context_input"] = value["modify_conditioning_matrix_at_context_input"]
        self.monophonic_network.modification_functions["modify_conditioning_matrix_at_monophonic_input"] = value["modify_conditioning_matrix_at_monophonic_input"]
        self.reverb_model.modification_functions["modify_ir"] = value["modify_ir"]
        #print(value)
    ...
    def compute_global_features(self, features, training):
        ...
        if self.z_encoder is not None:
            ...
            # add of modification functions
            features["global_inharm"] = tf.constant(self.modification_functions["modify_global_inharmonicity"](features["global_inharm"].numpy()))
            features["global_detuning"] = tf.constant(self.modification_functions["modify_global_detuning"](features["global_detuning"].numpy()))
            features["z"] = tf.constant(self.modification_functions["modify_z_vector"](features["z"].numpy()))
        if self.context_network is not None:
            features.update(self.context_network(features, training=training))
            # add of modification functions
            features["context"] = self.modification_functions["modify_context_matrix"](features["context"]) 
